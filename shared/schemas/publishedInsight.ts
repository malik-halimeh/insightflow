...
/**
 * What the owner actually fills in when publishing a finding. The id, slug and
 * publishedAt are assigned by the server — the slug also depends on a uniqueness
 * check against the collection, which a static schema cannot perform.
 */
export const publishedInsightCreateSchema = publishedInsightSchema.pick({
  displayName: true,
  caption: true,
  metricLabel: true,
  metricValue: true,
